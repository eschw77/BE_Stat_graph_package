...
plot_ising_true <- function(result,
  ...
  # Extract edge weights (theta values)
  ...
  # Prepare edge labels
  edge_labels <- if (show.weights) round(edge_weights, 3) else NA
...
plot_ising_estimated <- function(samples,
  ...
  if (nrow(estimated) > 0) {
    ...
    if (!is.null(edges) && nrow(edges) > 0) {
      ...
      is_correct <- estimated_pairs %in% edge_set
      edge_colors <- ifelse(is_correct, edge.color.correct, edge.color.false)
    } else {
